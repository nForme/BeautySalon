        ...
        private void btnConfirm_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                if (comboBoxReportType.SelectedIndex == 1)
                {
                    if (datePickerFrom.SelectedDate != null && datePickerUntill.SelectedDate != null)
                    {
                        if (datePickerFrom.SelectedDate < datePickerUntill.SelectedDate)
                        {
                            ...
                            lblNothingFound.Visibility = Visibility.Collapsed;
                            txtBlockRecordTitle.Text = "за период: " +
                                                       datePickerFrom.SelectedDate.Value.Date.ToShortDateString() +
                                                       " - " + datePickerUntill.SelectedDate.Value.Date
                                                           .ToShortDateString();
                            txtBlockRecordTitle.Visibility = Visibility.Visible;
                            DGridReportRecords.ItemsSource = BeautySalonBaseEntities.getContext().ClientServiceView
                                .Where(p => p.Date > datePickerFrom.SelectedDate &&
                                            p.Date < datePickerUntill.SelectedDate).ToList();
                        ...
                        {
                            MessageBox.Show("Начальная дата не можеть быть меньше конечной", "Информация",
                    ...
                    {
                        lblInitializePage.Visibility = Visibility.Collapsed;
                        DGridReportRecords.ItemsSource =
                            BeautySalonBaseEntities.getContext().ClientServiceView.ToList();
                    }

                    var items = DGridReportRecords.ItemsSource as List<ClientServiceView>;
                ...
                else
                {
                    ...
                    {
                        lblInitializePage.Visibility = Visibility.Collapsed;
                        var costFrom = Convert.ToDecimal(txtBoxPriceFrom.Text);
                        DGridReportServices.ItemsSource = BeautySalonBaseEntities.getContext().Service
                            .Where(p => p.Cost > costFrom).ToList();
                    }
                    ...
                    if (txtBoxPriceFrom.Text != "" && txtBoxPriceUntill.Text != "")
                    {
                        if (Convert.ToDecimal(txtBoxPriceFrom.Text) < Convert.ToDecimal(txtBoxPriceUntill.Text))
                        {
                            lblInitializePage.Visibility = Visibility.Collapsed;
                            var costFrom = Convert.ToDecimal(txtBoxPriceFrom.Text);
                            var costUntill = Convert.ToDecimal(txtBoxPriceUntill.Text);
                            DGridReportServices.ItemsSource = BeautySalonBaseEntities.getContext().Service
                                .Where(p => p.Cost > costFrom && p.Cost < costUntill).ToList();
                        ...
                        {
                            MessageBox.Show("Цена 'от' не может быть больше или равно 'до'", "Информация",
                                MessageBoxButton.OK, MessageBoxImage.Information);
                    ...
                    var items = DGridReportServices.ItemsSource as List<Service>;
